...
/// Тарифы ITDO Pro (раньше — «ИТДО ШЛЁП») — 1:1 с NUKSTA_PLANS в
/// assets/js/app.js веб-клиента. Бэкенд не отдаёт эти значения отдельным
/// эндпоинтом, поэтому они захардкожены здесь так же, как в вебе.
enum NukstaBillingPeriod: String, CaseIterable, Identifiable {
    case twoWeeks = "2w"
    case month
    case halfyear
    case year
    ...
    var daysLabel: String { "\(days) дней доступа" }
}

/// Список фич ITDO Pro — 1:1 со списком в .nuksta-price-card (assets/js/app.js).
enum NukstaPlan {
    static let features = [
